...
const MODS = (() => {
  ...
  function renderAtencion(session) {
    ...
    const pend    = fallas.filter(f => f.estatus === 'Pendiente' || f.estatus === 'En proceso' || f.estatus === 'Requiere refacción' || f.estatus === 'No localizado');
    const aten    = fallas.filter(f => f.estatus === 'Atendido');
      ...
      <div class="aten-split">
        ...
        <div class="aten-form-col">
          ...
          <div style="display:flex;flex-direction:column;gap:12px">
            ...
            <div class="form-group" id="atenFechaWrap" style="${f.estatus==='Atendido'?'':'display:none'}">
            ...
            <div class="form-group">
              ...
              <input type="text" id="atenTecnico" value="${f.tecnico || ''}" placeholder="Nombre del técnico">
            </div>
            <div class="form-group">
  ...
  function onAtenEstatusChange() {
    ...
    if (wrap) wrap.style.display = sel === 'Atendido' ? '' : 'none';
  }

  async function guardarAtencion() {
    if (!currentAtencion) return;
    const estatus   = document.getElementById('atenEstatus')?.value;
    const tecnico   = document.getElementById('atenTecnico')?.value.trim();
    const resultado = document.getElementById('atenResultado')?.value.trim();

    try {
      await DATA.actualizarReporte(currentAtencion, { estatus, tecnico, resultado });
    } catch (error) {
  ...
  function renderDashboard(session) {
    ...
    <div id="mod-dashboard" class="module active">
      ...
      <!-- Gráficas (ahora debajo de la tabla) -->
      <div class="charts-row">
        ...
        <div class="charts-side">
          <div class="card">
            <div class="card-hdr">
              <div class="card-icon card-icon-amber"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><path d="M12 2v10l7 3"/></svg></div>
          ...
          <div class="card">
            ...
            <div id="empresaBars" class="empresa-bars"></div>
          </div>
        </div>
      </div>
  ...
  function buildKPIs() {
    ...
    const aten   = fallas.filter(f => f.estatus === 'Atendido').length;
    const pct    = total ? Math.round(aten / total * 100) : 0;

    // Read live estatus list from config (dynamic)
    const estatusConfig = _getEstatusVivos();

    // Fixed 4 base KPIs always shown first
    const base4 = [
      { key: 'total',      val: total, lbl: 'Total Registros', sub: 'Histórico',             r: 79,  g: 142, b: 247 },
      { key: 'Pendiente',  val: fallas.filter(f=>f.estatus==='Pendiente').length,  lbl:'Pendientes',   sub:'Requieren atención', r:239,g:68,b:68 },
      { key: 'En proceso', val: fallas.filter(f=>f.estatus==='En proceso').length, lbl:'En Proceso',   sub:'En trabajo',         r:245,g:158,b:11 },
      { key: 'Atendido',   val: aten,  lbl: 'Atendidos',       sub: pct+'% del total',       r: 34,  g: 197, b: 94  },
    ];

    // Dynamic extra estatus (all configured except the 4 base ones)
    const base4Keys = new Set(['Pendiente','En proceso','Atendido']);
    const DYNA_COLORS = [
      [168,85,247],[6,182,212],[249,115,22],[132,204,22],[225,29,72],[16,185,129],[139,92,246],[236,72,153]
    ];
    const extras = estatusConfig
      .filter(e => !base4Keys.has(e))
      .map((e, i) => {
  ...
  function buildCharts() {
    ...
    if (mc) {
      // Use live config estatus list
      const estatusList = _getEstatusVivos();
      const total = fallas.length || 1;
      const aten  = fallas.filter(f => f.estatus === 'Atendido').length;
      const avancePct = Math.round(aten / total * 100);

      // Colors: fixed for base 4, cycled for extras
      const COLOR_MAP = {
        'Pendiente':          '#ef4444',
        'En proceso':         '#f59e0b',
        'Atendido':           '#22c55e',
        'Requiere refacción': '#a855f7',
        'No localizado':      '#06b6d4',
      };
      const EXTRA_COLORS = ['#ec4899','#f97316','#84cc16','#8b5cf6','#14b8a6','#e11d48','#10b981','#3b82f6'];
      let extraIdx = 0;
      const colors = estatusList.map(e => COLOR_MAP[e] || EXTRA_COLORS[extraIdx++ % EXTRA_COLORS.length]);
      const cntMap = {};
      estatusList.forEach(e => { cntMap[e] = fallas.filter(f => f.estatus === e).length; });

      mainChart = new Chart(mc, {
        type: 'bar',
        data: {
          labels: estatusList,
          datasets: [{ data: estatusList.map(e => cntMap[e]), backgroundColor: colors, borderRadius: 6, borderSkipped: false }]
        },
        options: {
      ...
      if (legend) {
        legend.innerHTML = estatusList.map((e, i) => {
          const pct = Math.round(cntMap[e] / total * 100);
          return `<div class="legend-item"><div class="legend-dot" style="background:${colors[i]}"></div><span>${e}: ${cntMap[e]} de ${total} (${pct}%)</span></div>`;
        }).join('');
      }
    ...
    if (cc) {
      const cats      = DATA.getSel('categoria');
      const catCounts = cats.map(c => fallas.filter(f => f.categoria === c).length);
      catChart = new Chart(cc, {
        type: 'doughnut',
        data: {
          labels: cats,
          datasets: [{ data: catCounts, backgroundColor: ['#4f8ef7','#22c55e','#f59e0b','#a855f7','#ef4444','#06b6d4'], borderWidth: 0 }]
        },
    ...
    if (tc) {
      ...
      const atendidos   = tecnicosSet.map(t => fallas.filter(f => f.tecnico === t && f.estatus === 'Atendido').length);
  ...
  function renderDashTable() {
    ...
    tbody.innerHTML = fallas.map(f => `
      <tr class="${UI.rowClass(f.estatus)}">
        ...
        ${canEdit ? `<td><button class="btn btn-ghost btn-sm" onclick="MODS.selAtencionFromDash('${f.id}')">Editar</button></td>` : ''}
      </tr>
  ...
  function renderAtendidosTable() {
    ...
    const session = AUTH.checkSession();
    let fallas = DATA.state.fallas.filter(f => f.estatus === 'Atendido');
  ...
  async function addComponente(emp, cat) {
    ...
    const added = [], skipped = [];
    values.forEach(val => {
      if (list.includes(val)) { skipped.push(val); return; }
      list.push(val);
      added.push(val);
    });
    if (added.length === 0) {
  ...
  async function addEmpresa() {
    ...
    const result = await DATA.agregarEmpresa(input?.value || '');
  ...
  return {
    ...
    selAtencion, selAtencionFromDash, guardarAtencion, onAtenEstatusChange, eliminarAtencion,
