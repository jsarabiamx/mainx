...
const DATA = (() => {
  ...
  async function init() {
    ...
    const [fallas, empresas, selectores, notificaciones, currentEmpresa, viewMode] = await Promise.all([
      DS.getAllReportesActivos(),
      DS.getEmpresas(),
      DS.getSelectores(),
      DS.getNotificaciones(),
      DS.getUiSetting('gho_current_empresa', 'GHO'),
      DS.getUiSetting('gho_view_mode', 'individual')
    ]);
  ...
  async function eliminarEmpresa(nombre) {
    ...
    // Validar: ¿tiene reportes activos?
    ...
    await saveSelectores(state.selectores);
    await AUTH.log('EMPRESA_DELETE', `Empresa eliminada (soft): ${nombre}`);
    return { ok: true };
  }
