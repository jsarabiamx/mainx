...
const APP = (() => {
  ...
  function updateNotifBadge() {
    const notifs = DATA.getNotificacionesNoLeidas().filter(n => n.destino === 'admin' || n.tipo === 'TECH_ATENDIENDO');
  ...
  function renderNotifList() {
    ...
    if (!cont) return;
    const all = DATA.getNotificaciones().filter(n => n.destino === 'admin' || n.tipo === 'TECH_ATENDIENDO').slice(0, 30);
