/* tecnico-ui.js — Visión + Perfil  v3.0
   Los paneles ya están en tecnico.html con display:none/block.
   Este archivo solo maneja: estilos compartidos, navegación y datos.
*/
(function () {
  'use strict';
  ...
  style.textContent = `
    ...
    .vp-tab {
      flex: 1; padding: 9px;
      background: #111622; border: 1px solid rgba(255,255,255,.1);
    ...
    .vp-tab.active {
      ...
      border-color: rgba(79,142,247,.4); color: #4f8ef7;
    }
    ...
    .vp-card-wide {
      background: #111622; border: 1px solid rgba(255,255,255,.07);
    ...
    .vp-prod-ring {
      ...
      border: 4px solid rgba(255,255,255,.07); flex-shrink: 0;
    }
    ...
    .vp-bar-fill { height: 100%; border-radius: 4px; }
    ...
    .vp-t-col { display: flex; flex-direction: column; align-items: center; justify-content: flex-end; gap: 3px; flex: 1; min-width: 0; }
    .vp-t-bar { width: 100%; border-radius: 3px 3px 0 0; min-height: 2px; }
    ...
    /* ── Estado vacío ── */
    .vp-empty { text-align: center; padding: 40px 16px; color: rgba(255,255,255,.3); font-size: 13px; }
    .vp-empty-ico { font-size: 36px; margin-bottom: 12px; }
    ...
    .vp-spinner { width: 18px; height: 18px; border: 2px solid rgba(255,255,255,.1); border-top-color: #4f8ef7; border-radius: 50%; animation: vpSpin .7s linear infinite; flex-shrink: 0; }
  ...
  /* ═══════════════════════════════════════════
     NAVEGACIÓN
     Cambia entre inicio / visión / perfil
     mostrando/ocultando paneles con display
  ═══════════════════════════════════════════ */
  window.tuiScreen = window.tuiGo = function (screen) {
    ...
    const navId = screen === 'vision' ? 'tniVision'
                : screen === 'perfil' ? 'tniPerfil'
                : 'tniInicio';
    document.getElementById(navId)?.classList.add('active');

    /* Mostrar / ocultar paneles */
    const pv = document.getElementById('tuiPanelVision');
    const pp = document.getElementById('tuiPanelPerfil');
    if (pv) pv.style.display = screen === 'vision' ? 'block' : 'none';
    if (pp) pp.style.display = screen === 'perfil' ? 'block' : 'none';
  ...
  async function getMyReportes() {
    if (_cache && Date.now() - _cacheTs < 60000) return _cache;
    ...
    try {
      ...
      console.warn('[TUI]', e);
      const fb = (window._techState?.fallas || []).filter(f =>
        (f.tecnicoUsername || '').toLowerCase() === un
      );
      _cache = fb;
      _cacheTs = Date.now();
      return fb;
  ...
  async function renderVision() {
    ...
    try {
      ...
      if (!s) {
        ...
        _vRendering = false; return;
      }

      const todos     = await getMyReportes();
      ...
      const perAtend   = enPer.filter(f => isAtendido(f.estatus));
      const perPend    = enPer.filter(f => isPendiente(f.estatus));
      const perProc    = enPer.filter(f => isEnProceso(f.estatus));
      const perCorrect = enPer.filter(f => /correctiv/i.test(f.tipo || ''));
      const perPrev    = enPer.filter(f => /preventiv/i.test(f.tipo || ''));

      const total     = todos.length;
      const totalPer  = enPer.length;
      ...
      const trend     = buildTrend(_period, todos);
      ...
      cont.innerHTML = `
        ...
        ${trend.some(d => d.val > 0) ? `
        <div class="vp-trend">
          <div class="vp-chart-title">Tendencia de atención</div>
          <div class="vp-trend-bars">
            ${trend.map(d => {
              const h = d.max > 0 ? Math.max(Math.round(d.val / d.max * 80), d.val > 0 ? 4 : 2) : 2;
  ...
  function buildTrend(period, reportes) {
    ...
    if (period === 'dia') {
      ...
      for (let w = 3; w >= 0; w--) {
        ...
        slots.push({ label: 'S'+(4-w), val: cnt });
      }
    } else {
  ...
  async function renderPerfil() {
    ...
    try {
      ...
      const nm    = p.nombre   || p.name     || '—';
      const un    = p.username                || '—';
      const em    = p.email                   || '—';
      const tel   = p.telefono                || '—';
      const base  = p.base                    || '—';
      const empId = p.empleado_id || p.empleadoId || '—';
      const emps  = p.empresas  || s.empresas || [];
      const role  = p.role      || s.role     || 'tecnico';
      ...
      const hist     = p.login_history || p.loginHistory || [];
      const lastLogin = hist.length > 0
        ? (() => {
            try {
              const e = hist[hist.length - 1];
              return new Date(e?.fecha || e?.ts || e).toLocaleDateString('es-MX',
      ...
      cont.innerHTML = `
        <div class="pp-wrap">
          ...
          <div class="pp-emp-chips">
            ${emps.length > 0
              ...
              : `<span class="pp-chip">${p.empresa || s.empresa || '—'}</span>`}
          </div>
        </div>

        <div class="pp-stat-grid">
          <div class="pp-stat"><div class="pp-stat-n">${totalR}</div><div class="pp-stat-l">Asignados</div></div>
          <div class="pp-stat"><div class="pp-stat-n" style="color:#22c55e">${atendR}</div><div class="pp-stat-l">Atendidos</div></div>
          <div class="pp-stat"><div class="pp-stat-n" style="color:#f59e0b">${pendR}</div><div class="pp-stat-l">Pendientes</div></div>
        </div>
        ...
        <div class="pp-card">
          <div class="pp-row"><span class="pp-row-lbl">📅 Registro</span><span class="pp-row-val">${fmt(p.created_at || p.createdAt)}</span></div>
