...
async function authLoginSuperAdmin(username, password) {
  ...
  // ── SA ya existe — validar con contraseña actual ──
...
async function ensureInitialMasterUser(username, password) {
  ...
  if (isSupabaseMode()) {
    ...
    return { ok: true, user: bootstrapSession, session: bootstrapSession, firstLogin: true, bootstrapMaster: true };
  }
...
function authCheckSession() {
  ...
  // Si es super admin, verificar que el dispositivo sigue siendo el autorizado
  if (session.isSuperAdmin && !session.recoveryMode) {
...
async function authCheckSessionAsync() {
  ...
  const session = STORE.session();
  if (!session) return null;
  if (session.isSuperAdmin && !session.recoveryMode && !isAuthorizedDevice()) {
    await clearSessionStore();
    return null;
  }
  if (isSupabaseMode() && !session.isSuperAdmin && !session.bootstrapMaster) {
    try {
      const authSession = await window.CCTV_SUPABASE.getAuthSession();
      if (!authSession || !authSession.user) {
        await clearSessionStore();
        return null;
      }
      if (session.userId !== authSession.user.id) {
        const profile = await window.CCTV_SUPABASE.getUserById(authSession.user.id);
      ...
      console.warn('[AUTH] checkSessionAsync supabase error:', e.message);
    }
  }
  return session;
}
...
function authRedirect(session) {
  ...
  if (role === ROLES.MASTER || role === ROLES.ADMIN || role === 'admin' || role === 'master') {
    window.location.href = 'app.html';
  } else if (role === ROLES.TECH || role === 'tecnico') {
