...
async function authTrackDeviceInfo() {
  ...
  if (!session.isSuperAdmin) {
    ...
    if (user) {
      ...
      user.login_history.unshift({ fecha: new Date().toISOString(), deviceType, browser, screenRes });
      if (user.login_history.length > 50) user.login_history.splice(50);
      await DS.updateUser(user.id, { login_history: user.login_history }, { usuario: session.username });
      AUTH_CACHE.users = await DS.getUsers();
    }
  }
}
